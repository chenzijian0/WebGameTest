...
public interface ReadUseableItemInf
{
    ...
    @Update("update usergameattributes set KFB = KFB + 100 where  id = #{id}")
    int SellItemType1(int id);

    @Update("update usergameattributes set KFB = KFB + 1000 where  id = #{id}")
    int SellItemType2(int id);

    @Update("update usergameattributes set KFB = KFB + 3000 where  id = #{id}")
    int SellItemType3(int id);
}
